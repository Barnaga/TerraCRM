...
void FinanceForm::createView(const QString& lbl)
{
	...
	getData("counterpars", 1, ui->payerBox, ui->communicationBox);
...
{
	int id;
	if (ui->payerBox->currentText().compare("Моя компания", Qt::CaseSensitive)==0) 
		id = ui->recipientBox->model()->index(ui->recipientBox->currentIndex(), 0).data().toInt();
	else 
		id = ui->payerBox->model()->index(ui->payerBox->currentIndex(), 0).data().toInt();
	query->prepare("SELECT * FROM project where \"companyID\" = :id");
