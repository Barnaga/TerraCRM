...
createProjectDialog::createProjectDialog(QWidget* parent, QSqlRelationalTableModel* model, QString id)
	: QDialog(parent), model(model), id(id)
{
	ui.setupUi(this);
	createComponyBox();
...
void createProjectDialog::getContactID(const int& row) {
	...
	//qDebug()<<ui.contact->model()->index(ui.contact->currentIndex(), 0).data().toString();

	//qDebug() << model->index(row, 0).data();
	//qDebug()<<ui.company->model()->index(ui.company->currentIndex(), 0).data().toString();
